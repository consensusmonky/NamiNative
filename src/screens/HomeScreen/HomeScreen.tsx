...
import { getCurrentAccount, getCurrentAccountIndex, getDelegation, getNetwork, setTxDetail, updateBalance } from '../../services/ApiConnectorService';
import { calculateAmount, fromAssetUnit, getTxDetail } from '../../utils/LedgerData';
...
export const HomeScreen = ({navigation, route}: any) => {
  ...
  async function updateTransactions(initialUpdate: boolean = false) {
    ...
    const transactionConfirmations = existingWallet[network.id]?.history.confirmed;
    if (!transactionConfirmations) {
      // await updateAccount(existingWallet, false, netInfo.isConnected === null || !netInfo.isConnected);
      return Promise.resolve(trans);
    }
   
    //});
    await Promise.all(transactionConfirmations.map(async (txHash: string) => {
      if (txHash == "") {
        return Promise.resolve(trans);
      }
      ...
      if (typeof existingWallet[network.id]?.history?.details[txHash] !== 'object' || Object.keys(existingWallet[network.id]?.history?.details[txHash]).length < 4) {
        let detail = await getTxDetail(txHash, initialLoadingReducer.status.offlineMessage.visible);
...
// console.log(route.params?.initialWallet)
  ...
  const showScanner = () => {
   ...
   const showWalletScreen = useMemo(() => {   
    return (
      <SafeAreaView style={[{flex: 1 }, isDarkMode ? {backgroundColor: Colors.darker} : {backgroundColor: Colors.lighter} ]}> 
        { isScannerVisible ? 
  ...
  const checkWallet = async(initialWallet = false) => {      
    ...
    // var test = await AsyncStorage.getItem(STORAGE.selectedNetwork) as string;
