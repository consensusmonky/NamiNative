...
export const updateAccount = async (existingAccount: AccountInfo | undefined = undefined, forceUpdate = false, offlineMode = false) => {
  ...
  const accounts = existingAccount ? [existingAccount] : Wallet.getArray<AccountInfo>(STORAGE.accounts);
  ...
  if (
    ...
    account[network.id]?.history?.confirmed[0] == account[network.id]?.lastUpdate &&
    !forceUpdate &&
    ...
    if (account[network.id]?.lovelace == null) {
      ...
      {
        account[network.id]!.lovelace = '0';
      }
      // var serializedAccount = JSON.stringify(account);
      Wallet.setArray(STORAGE.accounts, [account]);
...
export const setCollateral = async (collateral: any) => {
  ...
  const account = await getCurrentAccount() as AccountInfo;
  if (account[network.id]) {
    account[network.id]!.collateral = collateral;
  }
  var serializedAccount = JSON.stringify(account)        
